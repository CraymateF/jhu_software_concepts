...
def test_create_app_with_config():
    ...
    app = create_app(query_func=mock_query, config=config)
...
def test_analysis_page_with_invalid_database():
    ...
    # Invalid DB name is coerced to the default ('gradcafe') inside the route


@pytest.mark.buttons
def test_pull_data_with_invalid_database():
    """Test POST /pull-data returns 202 Accepted (task queued asynchronously)."""
    from app import create_app
    ...
    # module_6: buttons publish to RabbitMQ and return 202 Accepted
...
def test_pull_data_without_json():
    ...
    response = client.post('/pull-data')
    assert response.status_code in (202, 503)  # 202 if RabbitMQ reachable, 503 otherwise


@pytest.mark.web
def test_scraping_status_endpoint():
    """Test GET /scraping_status returns idle status (worker owns async state)."""
    from app import create_app
...
@pytest.mark.skip(reason="setup_databases.py not present in module_6")
@pytest.mark.db
