...
def get_db_params(dbname='gradcafe'):
    """
    ...
    if database_url:
        ...
        if '@' in database_url:
            ...
            if '/' in host_part:
                host_and_port, _ = host_part.split('/', 1)
                # Use parameter dbname (ignore URL dbname)
...
def get_existing_urls(dbname='gradcafe'):
    ...
    try:
        ...
        print(f"Error getting existing URLs: {exc}")
        return set()
...
def add_new_records_to_db(records, dbname='gradcafe'):
    ...
    try:
        ...
        print(f"Error adding records to database: {exc}")
        if conn:
            conn.rollback()
        return 0
    finally:
...
def scrape_and_update_background(dbname='gradcafe', max_pages=2):
    ...
    try:
        ...
        scraping_status["status_message"] = f"Error: {str(exc)}"
